...
type SplitFlapTextProps = Omit<HTMLAttributes<HTMLDivElement>, "children"> & {
  words?: string[];
  text?: string;
  flipDuration?: number;
  stagger?: number;
...
const resolveCharset = (charset: string) => {
  if (charset in CHARSETS) return CHARSETS[charset as keyof typeof CHARSETS];
...
const buildSequence = (target: string, flips: number, charset: string) => {
  const steps: string[] = [];
  for (let index = 0; index < flips; index += 1) steps.push(sampleChar(charset));
  steps.push(target);
  return steps;
};
...
export default function SplitFlapText({
  ...
  useEffect(() => {
    ...
    const animateTo = (targetPhrase: string) => {
      ...
      const plans = targetPhrase.split("").flatMap((target, index) => {
        const from = fromPhrase[index] || " ";
        return from === target
          ? []
          : [{ index, from, target, sequence: buildSequence(target, safeFlips, activeCharset), start: index * safeStaggerMs, step: -1, done: false }];
      });
