...
const measureLine = (ctx: CanvasRenderingContext2D, line: string, letterSpacing: number) => {
  ...
  return chars.reduce((width, char) => width + ctx.measureText(char).width, 0) + Math.max(0, chars.length - 1) * letterSpacing;
};

const drawLine = (ctx: CanvasRenderingContext2D, line: string, x: number, y: number, letterSpacing: number) => {
  const chars = Array.from(line);
...
const buildTextCanvas = (container: HTMLElement, width: number, height: number, dpr: number, props: RuntimeProps) => {
...
const syncUniforms = (program: Program, props: RuntimeProps) => {
...
export default function WarpText({
  ...
  const propsRef = useRef<RuntimeProps>({ text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple });
  const contextRef = useRef<RuntimeContext | null>(null);

  useEffect(() => {
    propsRef.current = { text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple };
    if (contextRef.current) {
      syncUniforms(contextRef.current.program, propsRef.current);
      contextRef.current.rasterize();
    }
  }, [text, color, fontSize, fontWeight, fontFamily, letterSpacing, lineHeight, warpStrength, warpScale, speed, pointerInfluence, pointerStrength, refraction, ripple]);
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      if (document.fonts?.ready) {
        try { await document.fonts.ready; } catch (error) { void error; }
      }
      if (disposed || contextLost || version !== rasterVersion) return;
      const rect = container.getBoundingClientRect();
      if (rect.width <= 0 || rect.height <= 0) return;
      texture.image = buildTextCanvas(container, rect.width, rect.height, Math.min(window.devicePixelRatio || 1, 2), propsRef.current);
